...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='data/logs2')
    ...
    parser.add_argument('--vae_dir', type=str, default='../auto_aug-master/results/emb64_0.1_5/model_new.pth')
    ...
    def run_iter(
        ...
        cifar_normalize = CIFARNORMALIZE(32)
        cifar_innormalize = CIFARINNORMALIZE(32)
        adv_inputs_list: List[torch.Tensor] = []
        for attack in step_attacks:
            attack_adv_inputs = inputs.clone()
            if to_attack.sum() > 0:
                xi = vae(cifar_normalize(inputs[to_attack]), mode="x-xi")
                xi = cifar_innormalize(xi.detach())
                xi = torch.clamp(xi, 0, 1)
                attack_xi = attack(xi, labels[to_attack])
                attack_adv_inputs[to_attack] = inputs[to_attack] - xi + attack_xi
            ...
            if iteration % 100 == 1:
                ...
                successful_attacks = []
                num_samples = min(to_attack.sum(), 10)
                for idx in range(num_samples):
                    successful_attacks.append(torch.cat([
                        inputs[to_attack][idx],
                        attack_adv_inputs[to_attack][idx],
                        inputs[to_attack][idx] - xi[idx],
                        xi[idx],
                        attack_xi[idx],
                        torch.clamp((attack_adv_inputs[to_attack][idx] -
                                     inputs[to_attack][idx]) * 3 + 0.5,
                                    0, 1),
                    ], dim=1).detach())
